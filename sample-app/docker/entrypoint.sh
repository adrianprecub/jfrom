#!/bin/sh
# Starts the sample app, attaching the jfrom agent when a jar is
...
AGENT_JAR="${AGENT_JAR:-/app/agent/jfrom-agent.jar}"
AGENT_OPTS="${AGENT_OPTS:-port=9404}"
AGENT_STARTUP_GRACE_SECONDS="${AGENT_STARTUP_GRACE_SECONDS:-3}"

if [ -f "$AGENT_JAR" ]; then
  echo "jfrom: attaching agent '$AGENT_JAR' with options '$AGENT_OPTS'"
  # shellcheck disable=SC2086
  java $JAVA_OPTS -javaagent:"$AGENT_JAR=$AGENT_OPTS" -jar "$APP_JAR" &
  PID=$!
  sleep "$AGENT_STARTUP_GRACE_SECONDS"
  if kill -0 "$PID" 2>/dev/null; then
    echo "jfrom: app is running with the agent attached"
    wait "$PID"
    exit $?
  fi
  wait "$PID"
  STATUS=$?
  echo "jfrom: process exited quickly (status $STATUS) while starting with the agent attached; retrying without -javaagent"
else
  echo "jfrom: no agent jar found at '$AGENT_JAR' - starting without -javaagent"
fi
